...
定义数据源需要遵守的公共接口，例如 `FundamentalsProvider`。
...
- 将 EDGAR DataFrame 转换成 `FundamentalRecord`。
...
定义不同基本面分析类型所需的最小输入模型，并负责从公共 `FundamentalRecord` 构建这些输入。
...
1. 在 `fundamentals` 中作为指标计算，不保存到原始 `FundamentalRecord`。
...
这样 `FundamentalRecord` 只保存从来源归一化得到的基础事实数据。
